...
SUB_CLASSES = {
    '10': [404, 751, 94, 281, 62, 207, 31, 949, 814, 569],
    'dogs': list(range(151, 251)),
}
...
# ######################################
# sampler
class TwoSourceBatchSampler(Sampler):
    def __init__(self, real_inds, syn_inds, batch_size, real_ratio=0.5):
        ...
        self.num_batches = math.ceil(len(self.real_inds) / self.batch_size)
        super().__init__(None)
...
class DistributedSampler(Sampler):
    ...
    def __iter__(self):
        if self.shuffle:
            ...
            indices = torch.tensor(self.indices)[torch.randperm(
                len(self.indices), generator=g)].tolist()
        else:
...
class DistributedTwoSourceSampler(
    ...
    def __init__(self,
        ...
        self.real_sampler = DistributedSampler(dataset.real_inds, num_replicas,
                                               rank, shuffle, seed, drop_last)
        self.syn_sampler = DistributedSampler(dataset.syn_inds, num_replicas,
                                              rank, shuffle, seed, drop_last)

        self.num_replicas = num_replicas
        self.rank = rank
        self.drop_last = drop_last
        self.num_samples = self.real_sampler.num_samples + self.syn_sampler.num_samples
        self.total_size = self.real_sampler.total_size + self.syn_sampler.total_size
    ...
    def __iter__(self):
        return [
            list(self.real_sampler.__iter__()),
            list(self.syn_sampler.__iter__())
        ]
    ...
    def set_epoch(self, epoch):
        ...
        self.syn_sampler.set_epoch(epoch)


class DistributedTwoSourceBatchSampler(DistributedTwoSourceSampler):
    def __init__(self,
                 dataset,
                 num_replicas=None,
                 rank=None,
    ...
    def __iter__(self):
        real_inds, syn_inds = super().__iter__()
        batch_sampler = TwoSourceBatchSampler(real_inds, syn_inds,
                                              self.batch_size, self.real_ratio)
        return iter(batch_sampler)

    def __len__(self):
        return math.ceil(self.real_sampler.num_samples / self.batch_size)
...
parser.add_argument('--sub-classes-split',
                    type=str,
                    default='10',
                    choices=['10', 'dogs'])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    sub_classes = SUB_CLASSES[args.sub_classes_split]
    data_name = f'imagenet-{args.sub_classes_split}'
    expr_name = f'{data_name}_resnet18_224x224' \
        f'_e{args.epochs}_bs{args.batch_size}_lr{args.lr:g}'
    ...
    model = ResNet18_224x224(num_classes=len(sub_classes))
    ...
    if args.dummy:
        ...
        LABEL_MAPPING = {v: i for i, v in enumerate(sorted(sub_classes))}

        def get_subset_dataset(dset):
            indices = np.in1d(dset.targets, sub_classes).nonzero()[0]
            new_samples = []
            new_targets = []
            for i in indices:
                new_samples.append(
                    (dset.samples[i][0], LABEL_MAPPING[dset.samples[i][1]]))
                new_targets.append(LABEL_MAPPING[dset.samples[i][1]])
            dset.samples = new_samples
            dset.targets = new_targets
            return dset

        train_dataset = get_subset_dataset(train_dataset)
        val_dataset = get_subset_dataset(val_dataset)

        if args.extra_data:
            extra_train_dataset = datasets.ImageFolder(args.extra_data,
                                                       transform=None)
            # sanity check
            for k, v in extra_train_dataset.class_to_idx.items():
                assert LABEL_MAPPING[int(k)] == v
    ...
    if args.distributed:
        if args.extra_data is None:
            ...
            train_batch_sampler = DistributedTwoSourceBatchSampler(
                train_dataset,
